...
export class QuizSystemManager {
     ...
     async setupInitialLevel() {
          if (!this.params.levelId && this.state.levels.length > 0) {
               const firstLevel = this.state.levels.find(level => level.attributes.level)
               if (firstLevel) {
     ...
     updateLevelUI() {
          ...
          const renderLevelQuestion = (question, isActive = false) => {
               const level = this.state.levels.find(level => level.id)
               ...
               <div class="start-quiz-btn flex items-center gap-x-2 w-fit cursor-pointer">
                    ...
                    sm:size-8 lg:size-11 ${activeClass}">
                         ...
                         ${question.id}${question.attributes.tipe}
                         </h5>
                    </div>
                    <h5 class="text-[10px] sm:text-[12px] lg:text-sm font-medium 
                    max-w-[6.5rem] sm:max-w-[8rem] md:max-w-[10rem] lg:font-semibold lg:max-w-full">
                         Bagian ${question.id}${question.attributes.tipe} - ${level.attributes.Topik}
                    </h5>
               </div>
               `
          ...
          this.elements.levelContainer.innerHTML = this.state.questions
               .map(question => renderLevelQuestion(question, question === this.currentQuestion))
               .join('')

          this.elements.levelSession.innerHTML = this.state.levels
               .map(level => renderLevelSession(level, level.id === this.state.activeLevel))
               .join('')
     }
     ...
     updateQuizUI() {
          ...
          const level = this.state.levels.find(level =>
               level.id.toString() === (this.params.levelId.toString() || this.state.activeLevel?.id.toString())
          )?.attributes

          if (level) {
               this.elements.levelDisplay.textContent = `#Bagian ${this.currentQuestion.id}${question.tipe} - ${level.Topik}`
          }
