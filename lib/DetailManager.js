export class DetailManager {
     ...
     async fetchProvincesData() {
          const provinces = await this.apiService.getProvinces(true)
     ...
     createFunFactCard(funFact) {
          const imageUrl = funFact?.gambar?.data?.attributes?.url || '/assets/images/cardIlustrasi.png'
          const title = funFact?.judul || `Fakta menarik provinsi ${this.provinceData.attributes.nama}`
          const description = funFact?.deskripsi || 'Jelajahi berbagai bahasa daerah yang tersebar di seluruh wilayah Indonesia!'
